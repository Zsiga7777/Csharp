...
await FileService.WriteToFileV1Async("informatika", itThemedBooks);
...
await FileService.WriteToFileV1Async("1900", booksFrom1900);
...
await FileService.WriteToFileV1Async("sorbarakott", orderedList);
...
List<Theme> booksOrderedByThemes = books.GroupBy(b => b.Theme)
    ...
    {
        ThemeName = b.Key,
        ListOfBookTitle = b.Select(b => b.Title).ToList()
    })
    .ToList();
await FileService.WriteToFileV1Async("kategoriak", booksOrderedByThemes);